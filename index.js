...
const split_word = () => {
  if (j < array[t].length) {
    document.getElementById("holder").innerHTML += array[t].charAt(j);
    j++;
...
const delete_word = () => {
  if (j >= 0) {
    let del = array[t].toString().slice(0, j);
...
// Navbar: smooth color transition

window.addEventListener('scroll', function() {
  const header = document.querySelector('header'); // Select the header element
  const scrollThreshold = window.innerHeight * 0.9; // 10% of the viewport height
